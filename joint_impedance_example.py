...
import logging
logging.basicConfig(
    filename="robot_example_torque.log",
    level=logging.INFO,
    filemode="w"
)
    ...
    try:
        ...
        for target_position in target_joint_positions:
            ...
            while True:
                ...
                logging.info("Measured link-side joint torques: %s", np.round(robot_state.tau_J, 4).tolist())
                logging.info("Calculated joint torques: %s", np.round(tau_d, 4).tolist())
