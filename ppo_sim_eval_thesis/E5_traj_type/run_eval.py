"""
E5 — Per-trajectory-type breakdown, models B / C / D vs HFDC baseline.

Models evaluated (μ=1.0, Fd=-8 N):
  B — Circle only, μ=1.0 fixed          (run_20260427_020537)
  C — Circle + Sinusoidal, μ random      (run_20260426_232324)
  D — All trajectory types, μ random     (random_trajectory_and_surface_friction)
...
from ppo_sim_eval_thesis.common import (
    CHECKPOINT_B, CHECKPOINT_C, CHECKPOINT_D,
    SLOPE_POS, R_SLOPE, SIZE_Z,
...
# condition key → (checkpoint prefix, display label)
CONDITIONS = {
    "hfdc":  (None,         "HFDC"),
    "ppo_B": (CHECKPOINT_B, "HFDC+PPO (B)"),
    "ppo_C": (CHECKPOINT_C, "HFDC+PPO (C)"),
    "ppo_D": (CHECKPOINT_D, "HFDC+PPO (D)"),
}
# ---------------------------------------------------------------------------
...
def main():
    ...
    # Pre-load all PPO agents once
    agents = {}
    for cond, (ckpt, _) in CONDITIONS.items():
        if ckpt is not None:
            agents[cond] = load_agent(ckpt)   # (agent, normalizer)

    trajs   = _make_trajs()
    results = {cond: {} for cond in CONDITIONS}

    for traj_name, (_, dur) in trajs.items():
        for cond, (ckpt, label) in CONDITIONS.items():
            agent, normalizer = agents.get(cond, (None, None))
            runs = []
            for r in range(N_RUNS):
                print(f"[E5] {traj_name}  {label}  run {r+1}/{N_RUNS}")
    ...
    meta = {
        "experiment": "E5",
        "description": (
            "Per-trajectory-type breakdown — models B/C/D vs HFDC baseline "
            "(μ=1.0, Fd=-8 N)"
        ),
        "conditions": {cond: {"checkpoint": ckpt, "label": lbl}
                       for cond, (ckpt, lbl) in CONDITIONS.items()},
        "n_runs": N_RUNS,
