...
def print_robot_state(state):
    ...
    try:
        ...
        rotation = state.O_T_EE[:12]
        ee = state.O_T_EE.copy()
        print(f"    Position: {np.round(position, 4).tolist()}")
        print(f"    Rotation: {np.round(rotation, 4).tolist()}")
        print(f"    ee: {type(ee)}")
    ...
    try:
        print(f"  Last commanded torques from controller: {np.round(state.tau_J, 4).tolist()}")
    except (AttributeError, TypeError):
